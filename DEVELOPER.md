...
If a new category is needed, add a directory under `Examples/Engineering/` or `Examples/Structural/` with an `_intro.md` file, explaining what the new category is about.
...
The entry's name must match the `.cpd`'s filename without the extension.
...
Latex is allowed in the description by surrounding the math with `$...$`.
Please note the `'` at the beginning.

`'<!-- ... -->`

The output is automatically rendered and displayed side-by-side with the CalcpadCE code.
See [Building the Documentation](#building-the-documentation) how to view the rendered example locally.
...
The CI build will fail if a difference is detected, and a unified diff is printed in the CI log showing what has changed.

If the change is intentional, the HTML stubs need to be regenerated to reflect the updated output.
...
Now, when filing a PR, the diff will show not only the code changes but also any resulting changes to the rendered output.
...
If you want to override this behavior (for whatever reason) and re-write all decimals, run the script with `--write --force`.

To run the comparison locally before filing the PR, run the script without any arguments.
Changes to any rendered output will be printed to the console.
